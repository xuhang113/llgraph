...
- 工具调用在编辑器里**跑之前就能看到**：模型一决定要调哪个工具就先出现一行（读哪个文件、跑哪条命令都写在标题上），真开跑变「进行中」，跑完才回填输出——一条 `run_terminal_cmd` 跑测试的几分钟里界面不再像卡住了。
  动文件的那几条还带上**受影响文件的绝对路径**，在编辑器里点一下就跳过去；工具没抛异常但返回像报错（参数校验错、`old_string` 没匹配上）时那一行标成**失败**，不会和成功的改写长得一样
- 改了代码的那条工具调用直接**展开成 diff**：点开就看到这一刀加了哪几行、删了哪几行（新建文件整份算新增），不用再去读「已写入 1234 字符」猜它干了什么
